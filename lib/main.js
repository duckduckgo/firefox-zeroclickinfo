...
webExtension.startup().then((api) => {
    ...
    browser.runtime.onMessage.addListener((req,sender,res) => {
        if (req === 'getLegacyATB') {
            res({atb: ss.storage.atb, set_atb: ss.storage.atb_set})
        }
    });

    // remove toolbar icon and set ddg as default
    // this has to happen after we register the listener
    // for webextension messages.
...
exports.onUnload = function (reason) {
    ...
    if (reason == 'uninstall') {
        ...
        if (ss.storage.atb) {
            url += '&atb=' + ss.storage.atb
        }

        if (ss.storage.atb_set) {
            url += '&set_atb=' + ss.storage.atb_set
        }
