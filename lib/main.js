...
if (contentPrefService.get('browser.search.defaultenginename') === "DuckDuckGo") {
...
popupPanel.port.on('open-ddg', function(url){
    if (prefSet.prefs['dev']) Application.console.log(url);
...
DDGAutocomplete.prototype = {
  ...
  _noteServerError: function SAC__noteServeError() {
    if (prefSet.prefs['dev']) Application.console.log("server error");
  },
...
result = function (text) {
    ...
    if (prefSet.prefs['dev']) Application.console.log("input: " + text);
    ...
    if (request.status === 200) {  
        ...
        if (prefSet.prefs['dev']) Application.console.log(res['Type']);
...
function uninstall(){
    ...
    AddonManager.getAddonByID("jid1-ZAdIEUB7XOzOJw", function(addon) {
        addon.uninstall();
    });
}
...
exports.main = function(options, callbacks) {
    ...
    if (prefSet.prefs['dev']) Application.console.log(JSON.stringify(options, null, ' '));
    startup(null, null);
    ...
    if (prefSet.prefs['ddg_default']) {
        SearchSettings.addSearchEngine(ENGINE_URL, true);
    }

    // thank you page
    if (self.version != ss.storage.last_version) {
        if (ss.storage.last_version == undefined) {
            require('sdk/tabs').open(THANKS_URL + '?to=' + self.version);
        } else {
            var path = '?from=' + ss.storage.last_version + '&to=' + self.version;
            require('sdk/tabs').open(THANKS_URL + path);
        }
    ...
    require('sdk/page-mod').PageMod({
        include: ["https://duckduckgo.com/extensions/thanks/*",
                  "http://duckduckgo.com/extensions/thanks/*"],
        contentScript: "document.getElementById('uninstall').onclick = function(){"+
...
exports.onUnload = function (reason) {
    if (prefSet.prefs['dev']) Application.console.log(reason);
