...
exports.main = function(options, callbacks) {
    ...
    webExtension.startup().then(api => {
        ...
        browser.runtime.onMessage.addListener(function(message, sender, reply){
            ...
            if(message.logLegacy) console.log(message.logLegacy)

            if (message == 'get-atb') {
                reply({atb: ss.storage.atb, set_atb: ss.storage.set_atb})
            }
...
exports.onUnload = function (reason) {
    ...
    if (reason == 'uninstall') {
        var atbDelta = atbgen.calc.atbDelta(ss.storage.atb),
            ...
            surveyURL = 'https://www.surveymonkey.com/r/' + extVersion + 'DOC_' + surveyUrlParam;
