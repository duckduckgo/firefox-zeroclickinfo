...
popupPanel.port.on('swap-default', function(def){
    ...
    if (def == true){
        ...
        contentPrefService.set("browser.search.defaultenginename", "DuckDuckGo");
    } else {
        SearchSettings.cleanup('uninstall');
        SearchSettings.addSearchEngine(ENGINE_URL, false);
    }
...
function loadPageMod() {
    ...
    pageModGoogle = pageMod.PageMod({
        ...
        contentScriptWhen: 'ready',
        contentStyleFile: [
            data.url("css/common.css"),
            data.url("css/google.css")
        ],
        contentScriptFile: [
...
function onPrefChange(prefName) {
    if (prefName == 'zeroclick') {
        ...
        if (prefSet.prefs['ddg_default'] === true) {
            ss.storage.prev_default_search_engine = contentPrefService.getLocalized("browser.search.defaultenginename");
            ss.storage.prev_selected_search_engine = contentPrefService.getLocalized("browser.search.selectedEngine");
            ...
            contentPrefService.set("browser.search.defaultenginename", "");

            console.log(ss.storage.prev_default_search_engine);
            console.log(ss.storage.prev_selected_search_engine);

            SearchSettings.cleanup('uninstall');
            SearchSettings.addSearchEngine(ENGINE_URL, false);

            contentPrefService.set("browser.search.defaultenginename", ss.storage.prev_default_search_engine);
        }
